...
It is clear that a solution to this problem breaks CSDIH, as it
...
is a minor departure from the original problem, and there is a sort of
...
\item On top of $E_1$, the SIDH protocol also publishes the evaluation
  ...
  compute the action of $\phi$ on any point of $E_0[e^m]$ (change $B$
...
indicated by so called \emph{torsion point attacks} on overstretched
variants of SIDH~\cite{AC:Petit17,EPRINT:KMPPS20}.
...
evaluating the strength of the relative assumptions. However using
...
reached~\cite{INDOCRYPT:BiaJaoSan14,BIJ18,Jao-etal-kuperberg-2018,10.1007/978-3-030-17656-3_15,EC:BonSch20,EC:Peikert20,cryptoeprint:2020:1520}.
...
Grover search runs in $O(p^{1/4})$ operations.
...
``torsion point attacks''~\cite{AC:Petit17,EPRINT:KMPPS20} seem so far
to only give an advantage against ``overstretched'' versions of SIDH
where the degrees of the isogenies are exponentially larger than
...
recently been analyzed and shown not to invalidate the security of the
SIKE parameters either~\cite{EPRINT:JaqSch20}.
